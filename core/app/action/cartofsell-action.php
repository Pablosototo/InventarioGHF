...
<tr >
  ...
  <td><?php echo $product->name; ?></td>
  <td><b><?php echo $symbol; ?> <?php echo number_format($product->price_in,2,".",","); ?></b></td>
  
  <td><b><?php echo Core::$symbol; ?> <?php $pt = $product->price_in * $p["q"];
																	$total += $pt;
...
<div class="col-md-6">
 ...
 <div class="col-md-6">
    ...
    <div class="col-lg-12">
      <input type="text" name="money" value=<?php echo ($total + ($total * ($iva_val / 100))) ?> class="form-control" id="money" placeholder="Efectivo">
    </div>
  </div>
  </div>
...
<script>
  $("#discount_percen").keyup(function(){
    $("#discount").val( ($("#discount_percen").val()/100)*<?php echo $total; ?>  );
      ...
      <input type="hidden" name="total" value="<?php echo ($total + ($total * ($iva_val / 100))) ?>" class="form-control" placeholder="Total">
...
<script>
    ...
    if(p!=4){
			...
			go = confirm("Cambio: $"+(money-(<?php echo ($total + ($total * ($iva_val / 100))) ?>-discount ) ) );
			if(go){
      e.preventDefault();
